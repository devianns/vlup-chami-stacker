...
- 총점은 `쌓은 차미의 크기 점수 합계 + 낮게 넣기 보너스(0~2,999)`입니다.
- 순위는 총점, 낮게 넣기 비율, 차미 개수, 달성 시각 순서로 비교합니다.
...
- 앱 흐름: [`src/app/StackerAppController.ts`](src/app/StackerAppController.ts)
- 화면 렌더링: [`src/ui/StackerAppView.ts`](src/ui/StackerAppView.ts)
- 반응형 UI: [`src/stacker-ui.css`](src/stacker-ui.css)
- 온라인 점수판 API: [`api/leaderboard.ts`](api/leaderboard.ts)
- 서버 점수 검증: [`api/leaderboard-validation.ts`](api/leaderboard-validation.ts)

현재 게임 프로토콜은 v5, 콘텐츠 버전은 3.0.1입니다. 작은·중간·큰 크기의 12가지 차미가 셔플 백에서 무작위로 등장하며, 크기별 점수와 낮게 넣기 보너스를 합산합니다. 한계선은 `Y=190`이고, 마찰·회전·초기 수평 속도를 조정해 이전보다 잘 구르고 미끄러집니다. 삐딱 차미는 투명 여백을 제외하고 흰색 몸통 외곽을 따르는 다각형 충돌체를 사용합니다.

UI는 `로딩 → 타이틀 → 플레이 → 결과/오류`의 단일 앱 단계로 관리합니다. 타이틀·인게임·결과 화면은 같은 DOM 뼈대를 사용하며, PC·세로 모바일·짧은 가로 화면은 요소를 복제하지 않고 CSS Grid 배치만 바꿉니다. 가로 휴대폰에서는 왼쪽 게임판과 오른쪽 HUD·시트리 패널의 2열 구조로 전환합니다. Phaser는 타이틀 UI가 표시된 뒤 별도 청크로 불러옵니다.
...
게임은 접속 직후 유휴 시간에 전체 순위표를 미리 받고, 순위창을 열 때 최신 기록을 다시 확인합니다. 겹친 요청은 순서를 추적해 늦게 끝난 예전 응답이 새 기록을 덮지 못하게 하며, 서버 응답이 늦거나 실패하면 브라우저 로컬 기록을 유지합니다.
...
- 서버는 JSON 게임 데이터에서 콘텐츠 버전·크기별 점수·보너스 한도를 직접 읽고, 크기별 개수·높이·낮게 넣기 비율과 체크섬을 검증합니다.
- 게임 장면은 순위표 전체를 저장하지 않고 완료 통계만 저장 관리자에 전달하므로, 다음 판이 끝나도 기존 닉네임과 기록을 덮어쓰지 않습니다.
